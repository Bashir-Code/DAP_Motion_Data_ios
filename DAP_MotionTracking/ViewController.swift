...
class ViewController: UIViewController {
    ...
    private var DapDataChunks = [DapDataChunk]() {
        didSet {
    ...
    private func exportAccelerometerData() {
        var result = ""
        for chunk in DapDataChunks {
            if !chunk.data.accelerometerSnapshots.isEmpty {
                result.append("\n\(chunk.createAcceletometerDataCSV())")
            }
        }
        
        shareStringAsFile(string: result, filename: "Dap-acceleration-\(Date()).csv")
    }
    ...
    private func exportGyroscopeData() {
        ...
        shareStringAsFile(string: result, filename: "Dap-gyroscope-\(Date()).csv")
    }
...
extension ViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = UITableViewCell(style: .default, reuseIdentifier: "DapSessionCell")
        let chunk = DapDataChunks[indexPath.row]
...
extension ViewController: WCSessionDelegate {
    ...
    func session(_ session: WCSession, didReceiveMessage message: [String : Any], replyHandler: @escaping ([String : Any]) -> Void) {
        ...
        let chunk = DapDataChunk.decodeIt(data)
        DispatchQueue.main.async {
            self.DapDataChunks.append(chunk)
        }
    }
